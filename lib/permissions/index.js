  ...
  var addShareToken = function(share) {
    ...
    if (share.expires > new Date()) {
      ...
      promise = AccountModel.findOneAsync({ email: share.userId })
        .then(function(account) {
          if (!account) {
            throw new Error("addShareToken - account not found for share: " + share.userId);
          }
    ...
    promise = AccountModel.findOneAsync({ email: share.userId })
      .then(function(account) {
        if (!account) {
          ...
          //throw new Error("removeShareToken - account not found for share: " + share.userId);
          errLog("removeShareToken - account not found for share: " + share.userId);
