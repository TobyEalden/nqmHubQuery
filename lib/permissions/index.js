...
module.exports = (function() {
            ...
            .then(function(accounts) {
              if (!accounts) {
                ...
                _.each(accounts, function(account) {
                  promises.push(addAccountAccess(account,resource.id,"read","public"));
                });
          ...
          return AccountModel.findAsync({id: {$in: accountIds}})
            .then(function(accounts) {
              if (!accounts) {
                ...
                _.each(accounts, function(account) {
                  promises.push(addSpecificAccountAccess(account,resource));
                });
    ...
    return ShareTokenModel.findAsync({ userId: account.email, owner: resource.owner, scope: resource.id })
      .then(function(shares) {
        ...
        _.each(shares,function(share) {
          ...
          promises.push(addAccountAccess(account, resource.id, access, share.id));
        });
      ...
      .then(function(trusts) {
        ...
        return AccountModel.findAsync({id: {$in: accountIds}})
          .then(function(accounts) {
            ...
            _.each(accounts, function(account) {
              promises.push(removeAccountAccess(account,resource.id,source));
            });
      ...
      promise = removeResourceAccess(resource,"public")
        .then(function() {
          if (resource.shareMode === "specific") {
            ...
            throw new Error("unknown share mode: " + resource.shareMode);
          }
        });
    } else {
  ...
  var addResource = function(resource) {
    return AccountModel.findOneAsync({id: resource.owner})
      .then(function(account) {
        if (!account) {
          throw new Error("no account found for resource owner: " + resource.owner);
        }
  ...
  var shareTokenAdded = function(share) {
    var promise;
      ...
      promise = AccountModel.findOneAsync({ email: share.userId })      
        .then(function(account) {
          if (!account) {
            ...
            //throw new Error("shareTokenAdded - account not found for share: " + share.userId);
  ...
  var shareTokenRemoved = function(share) {
    var promise;
    ...
    promise = AccountModel.findOneAsync({ email: share.userId })
      .then(function(account) {
        if (!account) {
          // TODO - this might be a share token for an external user - ignore for now.
          //throw new Error("shareTokenRemoved - account not found for share: " + share.userId);
  ...
  return {
    ...
    addShare: shareTokenAdded,
    removeShare: shareTokenRemoved
  }
}());