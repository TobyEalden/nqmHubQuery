...
module.exports = (function() {
  ...
  var log = require("debug")("nqmQuery:permissions");
  var errLog = require("debug")("nqmQuery:permissions:error");
    ...
    if (resource.shareMode !== "public") {
      ...
      promise = removeResourceAccess(resource,"public")
        .then(function() {
          ...
          } else if (!resource.shareMode || resource.shareMode === "private") {
    ...
    return AccountModel.findOneAsync({id: resource.owner})
      .then(function(account) {
        ...
        return removeAccountAccess(account, resource.id, "owner")
          .then(function() {
            // Remove reference to the resource from all accounts.
            return removeResourceAccess(resource);
          })
          .then(function() {
            // Remove all share tokens for the resource.
            return ShareTokenModel.removeAsync({scope: resource.id})
          })
          .return(resource);
      });
  };
  ...
  var addTrustedZone = function(trustedZone) {
    ...
    if (trustedZone.status === "trusted" && trustedZone.expires > Date.now()) {
      ...
      var ds = ResourceModel.findAsync({owner: trustedZone.owner,shareMode: "public"});
      promise = Promise.join(ta,ds,function(trustedAccount,docs) {
        ...
        .spread(function(trustedAccount, shareTokens) {
          var promises = _.map(shareTokens, function(share) {
            // Check that the resource is in the correct mode.
            return ResourceModel.findOneAsync({id: share.scope})
              .then(function(shareResource) {
                if (!shareResource) {
                  // TODO - re-instate this once the database is cleaned.
                  return Promise.resolve(trustedAccount); //(new Error("resource not found: " + share.scope));
                }
                // Only add account access if the resource is in specific share mode.
                if (shareResource.shareMode === "specific") {
                  return addAccountAccess(trustedAccount, share.scope, getShareTokenAccess(share), share.id);
                }
                return Promise.resolve(trustedAccount);
              })
          });
          return Promise.all(promises);
        });
    ...
    if (trustedZone.status === "trusted") {
      ...
      promise = AccountModel.findOneAsync({id: trustedZone.other })
        .then(function(trustedAccount) {
          ...
          // Remove all resources owned by the zone owner from the trusted account.
          return ResourceModel.findAsync({owner: trustedZone.owner })
            .then(function(docs) {
              _.each(docs, function(doc) {
