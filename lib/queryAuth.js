...
module.exports = (function() {
  ...
  var _queryModels = _queryDB.models;
  ...
  var grantClientToken = function (credentials, req, cb) {
    ...
    _authServer().lookupShare(shareId, secret)
      .then(function(share) {
        ...
        if (share && share.status !== constants.trustedStatus) {
  ...
  var authenticateToken = function (id, req, cb) {
    _queryModels().ApiTokenModel.findOneAsync({id: id})
      .then(function(apiToken) {
        ...
        if (!apiToken) {
          ...
          if (apiToken.shareId) {
            _queryModels().ShareTokenModel.findOneAsync({id: apiToken.shareId, status: constants.trustedStatus, expires: {$gt: new Date()} })
              .then(function(share) {
                if (!share) {
              ...
              .catch(function(err) {
                errLog("failure looking up share %s",apiToken.shareId);
                cb(null, false);
              })
          } else {
            req.clientId = authData;
