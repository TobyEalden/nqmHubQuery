...
module.exports = (function() {
  ...
  var setSyncTimestamp = function(model, t) {
    log("saving sync timestamp for %s", model);
  ...
  var processQueue = function(queue) {
    ...
    if (queue.length > 0) {
      ...
      return self.onEvent(nextEvent)
        .then(function() { setSyncTimestamp(self._key, nextEvent.__timestamp); })
        .then(function() {
  ...
  var liveHandler = function(data) {
    ...
    if (this._catchingUp) {
      ...
      log("%s - queued event received during catch-up", this._ModelClass.modelName);
    } else {
