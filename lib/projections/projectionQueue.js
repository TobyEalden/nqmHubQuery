...
module.exports = (function() {
  var log = require("debug")("ProjectionQueue");
    ...
    SyncStatusModel.findOneAsync({ model: this._key })
      .then(function(sync) {
        if (sync) {
          ...
          eventBus.catchUp(self._key, sync.timestamp, catchUpHandler.bind(self));
        } else {
          // Issue catchup request for all events.
          eventBus.catchUp(self._key, 0, catchUpHandler.bind(self));
        }
      });

    // Listen for dataset events.
    return eventBus.subscribe(self._key + ".*", liveHandler.bind(self));
  }
