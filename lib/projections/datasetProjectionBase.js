...
module.exports = (function() {
  var log = require("debug")("datasetProjectionBase");
    ...
    if (this.isCatchingUp()) {
      ...
      if (dataset.scheme && dataset.scheme.fields && dataset.scheme.fields.length > 0) {
        ...
        return this._dataProjectionCache[dataset.store].start(dataset)
          .catch(function(err) {
            log("failure loading data for %s [%s]",dataset.store, err.message);
          });
      } else {
  ...
  DatasetProjectionBase.prototype.onEvent = function(data) {
    ...
    switch (data.__event) {
      case "created":
        ...
        break;
      case "renamed":
        promise = self._ModelClass.findOneAsync(lookup).then(function(persist) { persist.name = data.name; persist.version = data.__version; return persist.saveAsync(); });
        break;
      case "schemaSet":
      ...
      case "deleted":
        ...
        break;
      default:
        log("unrecognised %s event: %s", self._ModelClass.modelName, data.__event);
