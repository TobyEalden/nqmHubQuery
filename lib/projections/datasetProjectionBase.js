  ...
  DatasetProjectionBase.prototype.onEvent = function(data) {
    ...
    switch (data.__event) {
      case "created":
        ...
        promise = persist.saveAsync().then(function() { return self.startDataProjection(persist); }).catch(persistError);
        break;
      ...
      case "schemaSet":
        promise = self._ModelClass.findOneAsync(lookup)
          .then(function(persist) { persist.scheme = data.schema; persist.uniqueIndex = data.uniqueIndex; persist.version = data.__version; return persist; })
          .then(function(persist) { return persist.saveAsync().return(persist); })
