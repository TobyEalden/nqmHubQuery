...
exports.Listener = (function() {
  ...
  var log = require("debug")("nqmQueryHub:httpQueryListener");
  var errLog = require("debug")("nqmQueryHub:httpQueryListener:error");
  ...
  var startServer = function() {
    ...
    server.get(apiVersion + "/:uid/datasets/:id", function(req, res, next) {
      nqmAuth.authenticate(req, res, next, function(accessToken) {
        ...
        // Need to check that the user has access to the
        // requested user/dataset combination.
        nqmAuth.authorised(accessToken, uid, id, function(err, shareToken) {
          if (err || !shareToken) {
            res.statusCode = 401;
    ...
    server.get(apiVersion + "/:uid/datasets/:id/data", function(req, res, next) {
      nqmAuth.authenticate(req, res, next, function() {
        var id = req.params.id;
