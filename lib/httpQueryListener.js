...
exports.Listener = (function() {
  ...
  var startServer = function() {
      ...
      function(req,res,next) {
        ...
        _queryDB.models().AccountModel.findOneAsync({email: req.user.email})
          .then(function(account) {
            ...
            return _queryDB.models().ShareTokenModel.findOneAsync({scope: account.id, subject: account.email, status: constants.trustedStatus, expires: {$gt: Date.now() } });
          })
          .then(function(share) {
            if (!share) {
            ...
            apiToken.subject = req.user.email;
            ...
            apiToken.saveAsync()
              .then(function() {
                ...
                var redirectTo = decodeURIComponent(req.query.state + "?access_token=" + apiToken.id);
    ...
    var resourceServer = function(server, req, res, next) {
      ...
      return _queryDB.models().ResourceModel.findOneAsync({id: id})
        ...
        .then(function(resource) {
          if (resource.shareMode === constants.publicReadShareMode || resource.shareMode === constants.publicWriteShareMode) {
            // Resource is public - permit.
            log("permitting access to %s - resource is public" ,id);
            return server(id);
          } else {
            // Check for bearer token.
            return queryAuth.authenticate(req, res, next, function(err, capability) {
              if (err || !capability) {
              ...
              return queryAuth.authorised(resource, capability)
                .then(function(resource) {
                  if (!resource) {
                    res.statusCode = 401;
    ...
    server.get(apiVersion + "/datasets/:id", function(req, res, next) {
      ...
      return resourceServer(serveResource, req, res, next);
    });
