...
exports.Listener = (function() {
  ...
  var authServer = require("./authServer").api;
  ...
  QueryListener.prototype.start = function(config,models) {
    this._config = config;
    this._models = models;
  ...
  var startServer = function() {
    ...
    server.get("/auth/google/callback", passport.authenticate("google", { session: false, failureRedirect: "/authenticate" }),
      function(req,res,next) {
        ...
        // Find user account.
        self._models.AccountModel.findOneAsync({email: req.user.email})
          .then(function(account) {
            ...
            return self._models.ShareTokenModel.findOneAsync({scope: account.id, userId: req.user.email, status: "trusted", expires: {$gt: Date.now() } });
          })
          .then(function(share) {
            if (!share) {
            ...
            log("get API token from command oauth endpoint", params);
