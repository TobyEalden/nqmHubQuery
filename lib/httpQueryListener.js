...
exports.Listener = (function() {
  ...
  var _apiTokenTimeout = 60*60*1000;    // Default to 1hr.
  var _APIKey = "2snfDAfVHnZM";         // TODO - shared with toolbox?
  ...
  var startServer = function() {
    ...
    server.get("/auth/google", function(req,res,next) {
      ...
      var authFunc = passport.authenticate("google", { session: false, state: returnURL, scope: "https://www.googleapis.com/auth/userinfo.email" });
    ...
    server.get("/auth/google/callback", passport.authenticate("google", { session: false, failureRedirect: "/authenticate" }),
      function(req,res,next) {
        ...
        // Create API token for email.
        var apiToken = {
          iss: "google",
          sub: req.user.email,                // Subject is the email of the claimant
          jti: shortId.generate(),            // ID of this token.
          exp: moment().add(_apiTokenTimeout, "minutes").valueOf(),
        };
        // Referer is the IP address of the claimant.
        // TODO - spoofing check - find 3rd party module
        apiToken.ref = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
        log("sending apiToken create command: %j", apiToken);
        request.post({ url: "http://localhost:3102" + "/command/apiToken/create", method: "post",  json: true, body: apiToken }, function(err, response, content) {
          if (err || response.statusCode !== 200) {
            errLog("failure creating API token. Status code: %d, error: %s", response.statusCode, err.message);
            next(err);
          } else {
            log("api token created: %s",content.result.jti);
            var redirectTo = decodeURIComponent(req.query.state + "?access_token=" + content.result.jti);
    ...
    server.get(apiVersion + "/datasets/:id", function(req, res, next) {
      ...
      nqmAuth.authenticate(req, res, next, function(accessToken) {
        // Got a valid token - get request resource Id.
        var id = req.params.id;

        // Need to check that the authenticated user has access to the
        // requested resource.
        nqmAuth.authorised(accessToken, id)
          .then(function(shareToken) {
            if (shareToken.length === 0) {
              res.statusCode = 401;
              res.contentType = "text/plain";
              res.send("not authorised - no share token found");
              next();
            } else {
