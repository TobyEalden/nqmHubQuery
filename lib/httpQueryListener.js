    ...
    var getDatasetData = function(id, req, res, next) {
      ...
      return getQueryParams(req)
        .then(function(queryParams) {
          return queryAPI.getDatasetData(id, queryParams.filter, queryParams.projection, queryParams.options)
            .then(function(data) {
              var ret = {
    ...
    server.get(apiVersion + "/datasets", function(req, res, next) {
      ...
      return queryAuth.authenticate(req, res, next)
        .then(function (authData) {
          ...
          return _queryDB.models().AccountModel.findOneAsync({email: authData.accessToken.subject});
        })
        .then(function (account) {
          if (!account) {
