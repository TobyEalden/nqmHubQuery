  ...
  var startServer = function() {
    ...
    server.get("/auth/google/callback", passport.authenticate("google", { session: false, failureRedirect: "/authenticate" }),
      function(req,res,next) {
        log("google auth callback - user is %s", req.user.username);

        // Find user account for the logged in user.
        _queryDB.models().AccountModel.findOneAsync({id: req.user.username})
          .then(function(account) {
            if (!account) {
              errLog("no account for user %s", req.user.username);
          ...
          .then(function(share) {
            if (!share) {
              errLog("no account capability for %s", req.user.username);
