...
module.exports = (function() {
  ...
  var getFeedData = function(hubId, feedId, from, to, sortBy, sortDir, limit, skip, cb) {
    ...
    IOTFeedModel.findOneAsync({ hubId: hubId, id: feedId })
      .then(function(feed) {
        if (feed) {
          var DataModel = DataModelFactory.createModel(feed.store, feed.scheme.fields);
          DataModel.find(query, fields, options, cb);
        } else {
          log("feed '%s' not found at hub '%s'", feedId, hubId);
  ...
  var getDatasetData = function(dsId, sortBy, sortDir, limit, skip, cb) {
    ...
    DatasetModel.findOneAsync(query)
      .then(function(dataset) {
        if (dataset) {
          var DataModel = DataModelFactory.createModel(dataset.store, dataset.scheme.fields);
          DataModel.find({}, fields, options, cb);
        } else {
          log("dataset '%s' not found", dsId);
          cb(new Error("not found"));
        }
      });
  };
