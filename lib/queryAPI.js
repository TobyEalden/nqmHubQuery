...
module.exports = (function() {
  ...
  var createDerivedLookup = function(dataset, filter) {
    if  (dataset.derived && dataset.derived.source) {
      var lookup = { $and: [] };
      if (filter) {
        lookup.$and.push(filter);
      }
      return doCreateDerivedLookup(dataset, lookup);
    } else {
      return Promise.resolve({ lookup: filter, sourceDataset: dataset });
    }
  };
  
  var doCreateDerivedLookup = function(dataset, lookup) {
    return _queryModels().DatasetModel.findOneAsync({id: dataset.derived.source})
      .then(function(sourceDataset) {
        ...
        var createLookup = function(source) {
          ...
          return { lookup: lookup, sourceDataset: source};
        };
        
        if (sourceDataset.derived.source) {
          return doCreateDerivedLookup(sourceDataset, lookup).then(function(source) { return createLookup(source.sourceDataset); });
        } else {
  ...
  var getDatasetData = function(dsId, filter, projection, options) {
    return _queryModels().DatasetModel.findOneAsync({id: dsId})
      ...
      .then(function(derivedLookup) {
        ...
        return DataModel.find(derivedLookup.lookup, projection, options);
      })
      .catch(function(err) {
